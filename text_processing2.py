...
def digits_to_words(input_string):
    ...
    * 만약 인풋 스트링에서 숫자가 존재하지 않는 다면, 빈 문자열 (empty string)을 반환함
    ...
    digit_string = ""
    for nb in input_string:      
      if(nb.isdigit()):
        digit_string += str_digits[int(nb)] + " " 
...
def to_camel_case(underscore_str):
    ...
    * 만약 어떤 변수 이름이 underscore로만 이루어 진다면, 빈 문자열만 반환해도 됨
        ...
        Examples:
            ...
            "alreadyCamel"
    """
    if(len(underscore_str.lower().split("_")) != 1):
      split_str = underscore_str.lower().split("_")
      camelcase_str = ""
      i = 0
      for wd in split_str:
        if(i != 0):
          camelcase_str += wd.capitalize()
        elif(wd != ""):
          camelcase_str += wd
          i += 1
    else:
